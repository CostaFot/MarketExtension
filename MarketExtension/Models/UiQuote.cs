...
internal sealed record UiQuote(DomainQuote Source)
{
    ...
    // Currency pairs are quoted to 4 decimals (e.g. 1.0832); stocks/crypto as money. Invalid
    // quotes (unknown symbol, unavailable, or a fetch failure) render as a dash.
    public string FormatPrice() => !IsValid
        ...
        : Category == AssetCategory.Currency
            ...
            : Source.Price.ToString("$#,##0.00", CultureInfo.InvariantCulture);
