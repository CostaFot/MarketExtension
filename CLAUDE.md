...
- **Done (latest): migrated the `PricedListPage` trio (`WatchlistPage`/`FavoritesPage`/`PortfolioPage`) to
  ...
  **deleted**: the fields (`_priceCache`/`_cacheLock`/`_snapshot`/`_received`/`_fetchGeneration`/
  ...
  detail page open at a time → no cross-surface drift). Build clean (0 warnings); **not yet live-verified
...
- **`MarketRepository` orchestration** — owns one `IQuoteCache`. `GetQuotesAsync`/`RefreshAsync` write through
  (the single cache-fill path, now driven by the poll loop + the observe-subscribe fetch; note `GetQuotesAsync`
  has **no direct surface callers** anymore — every surface observes, and the observe/poll paths fetch via
  `RefreshAsync`/`RefreshSafe`, so the public `GetQuotesAsync` is kept only as coordinator API/for tests).
